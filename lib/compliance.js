...
function buildBundle(cwd, opts) {
  ...
  const bundle = {
    kind: "darknode.compliance.bundle",
    version: 1,
...
function verifyBundle(bundle, signingKey) {
  ...
  let sigOk = null;
  if (sig && signingKey) sigOk = sig.value === crypto.createHmac("sha256", signingKey).update(canon).digest("hex");
  return { hashOk, sigOk };
}
...
function renderBundleMd(b) {
  const u = b.usage || {};
  const L = [];
  L.push("# Darknode compliance report — " + b.project);
  L.push("");
  ...
  L.push("");
  L.push("_Verify with: `darknode compliance verify <file>`_");
  return L.join("\n");
}
