...
const sections = {
  ...
  update(el) {
    ...
    const openDl = () => S.openExternal("https://darknode.ai");
    const run = async () => {
...
<span class="c-ok">✓</span> node-pty ready · <span class="c-ok">✓</span> tools · <span class="c-ok">✓</span> ollama proxy`;
  ...
  async vms(el) {
    ...
    el.innerHTML = `
      ...
      <div class="card" style="max-width:1000px">
        ...
        <div class="pb-fields" style="grid-template-columns:2fr 1fr 1fr 1fr">
          <label class="pb-f"><span>Name</span><input class="in" id="vmName" placeholder="kali" spellcheck="false"></label>
          <label class="pb-f"><span>Memory (MB)</span><input class="in" id="vmMem" value="2048" spellcheck="false"></label>
          <label class="pb-f"><span>CPUs</span><input class="in" id="vmCpu" value="2" spellcheck="false"></label>
          <label class="pb-f"><span>Disk (GB)</span><input class="in" id="vmDisk" value="20" spellcheck="false"></label>
        </div>
    ...
    $("#vmCreate", el).onclick = async () => {
      const msg = $("#vmCreateMsg", el); msg.className = "run-status"; msg.textContent = "creating disk...";
      const r = await S.vmCreate({ name: $("#vmName", el).value.trim(), memMB: $("#vmMem", el).value, cpus: $("#vmCpu", el).value, diskGB: $("#vmDisk", el).value, iso: $("#vmIso", el).value.trim() });
  ...
  settings(el) {
    ...
    { const b = $("#ab-site", el); if (b) b.onclick = () => S.openExternal("https://darknode.ai"); }
    { const b = $("#ab-docs", el); if (b) b.onclick = () => S.openExternal("https://darknode.ai/docs"); }
